...
void comment_read(string inputstring, char(&idch)[8], int firstletter)
{
	...
	case 2:
	{
		if (size < 8)
		{
			for (unsigned long i = 2; i < (unsigned long)size; ++i)
			{
	...
	case 3:
	{
		...
		if ((size < 9) && (size > 3))
		{
			for (unsigned long i = 3; i < (unsigned long)size; ++i)
			{
	...
	case 4:
	{
		if (size < 10)
		{
			for (unsigned long i = 4; i < (unsigned long)size; ++i)
			{
				idch[i - 1] = inputstring[i];
			}
			empty_till_end(idch, size - 1);
...
element AtomicTypeTable::get_element(size_t Id)
{
	if (Id > (int)(elements_.size() + 1))
	{
...
{
	if ((Id < 0) && (Id > (int)(elements_.size() + 1)))
	{
		cout << "not found such Id in Table" << endl;
		system("pause");
	}
