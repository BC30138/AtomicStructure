	...
	const vector<int> &hyst, bool is_frequency)
{
	...
	if (is_frequency)
	{
		for (unsigned int i = 0; i < (unsigned int)hyst.size(); i++)
		{
			if (hyst[i] != 0)
	...
	plot << "bin(x, s) = s*int(x / s) + width / 2" << endl;
	...
	plot << "plot 'data/" << filename << "' u (bin(column(1), width)):(column(2)) w boxes fs solid 0.7 lt rgb 'purple' title '" << HystName << "'"
		<< endl;
	plot << "pause - 1" << endl;
